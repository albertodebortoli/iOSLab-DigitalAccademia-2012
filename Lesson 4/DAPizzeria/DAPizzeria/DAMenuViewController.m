...
- (void)viewDidLoad
{
    ...
    for (NSInteger i = 0; i < [[self sharedModel].menu.pizzas count]; i++) {
        DAPizza *pizza = ([self sharedModel].menu.pizzas)[i];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notificationReceived:) name:@"ingredientQuantity" object:nil];
}
